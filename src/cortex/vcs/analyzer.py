...
def get_file_lineage(workspace, file_path):
    ...
    try:
        ...
        output = subprocess.check_output(
            cmd, cwd=real_root, stderr=subprocess.STDOUT, timeout=15
        ).decode().strip()
        if not output:
            return None
...
def get_file_history(workspace, file_path, limit=5):
    ...
    try:
        ...
        sys.stderr.write(
            f"DEBUG GIT: workspace={workspace}, real_root={real_root}, rel_path={rel_path}\n"
        )
...
def install_git_hooks(workspace):
    ...
    scripts = {
        "post-checkout": "#!/bin/sh\nROOT=$(git rev-parse --show-toplevel)\ncortex-watcher index --workspace \"$ROOT\" &\n",
        "post-merge": "#!/bin/sh\nROOT=$(git rev-parse --show-toplevel)\ncortex-watcher index --workspace \"$ROOT\" &\n",
    }
