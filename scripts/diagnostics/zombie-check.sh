...
target_regex="$(printf '%s\n' \
    "$repo_root/rust/target/debug/cortex-engine" \
    "$repo_root/rust/target/debug/cortex-watcher" \
...
worker_pids=$(pgrep -f "cortex-engine|cortex-watcher|engine_worker.py" || true)
